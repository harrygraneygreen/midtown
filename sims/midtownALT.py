...
#another function to return the array of vn's and xn's
...
sumoCmd = [sumolib.checkBinary('sumo-gui'), '-c', 'midtownALT.sumo.cfg']
traci.start(sumoCmd)
step = 0

traci.route.add(routeID = 'route1', edges = ['-25', '-26', '-27', '-28', '-29', '-30', '-32', '-35', '-37'])
#traci.vehicle.add('ego', "route")
traci.vehicle.add('ego', "route1", departPos=str(200), departSpeed=str(spd), departLane = str(3))
    ...
    if step==0:
        ...
        for i in range(N_VEHICLES):
            if i < N_lane_1:
                ...
                vid = "%d" % i
                traci.vehicle.add(vid, "route1", departPos=str(position[0, i]), departSpeed=str(spd), departLane = str(3), typeID="vtypeauto")
                traci.vehicle.setColor(vid, (random.uniform(0, 255), random.uniform(0, 255), random.uniform(0, 255), 255))
                traci.vehicle.setLaneChangeMode(vid, 256)
